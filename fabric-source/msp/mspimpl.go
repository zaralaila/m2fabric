...
func (msp *bccspmsp) getSigningIdentityFromConf(sidInfo *m.SigningIdentityInfo) (SigningIdentity, error) {
	...
	if err != nil {
		...
		// Try loading as PQC key first (inferred from key size)
		pqcSign, pqcErr := loadPQCSignerFromPEM(sidInfo.PrivateSigner.KeyMaterial)
		if pqcErr == nil {
			return newPQCSigningIdentity(idPub.(*identity).cert, idPub.(*identity).pk, pqcSign, msp)
		}
...
func (msp *bccspmsp) sanitizeCert(cert *x509.Certificate) (*x509.Certificate, error) {
	var err error
	// PQC certificates have unknown public key algorithms — skip ECDSA sanitization.
...
func (msp *bccspmsp) IsWellFormed(identity *m.SerializedIdentity) error {
	...
	// PQC certificates have unknown public key algorithm; skip canonical ECDSA signature check.
	if cert.PublicKeyAlgorithm == x509.UnknownPublicKeyAlgorithm {
		return nil
	}
