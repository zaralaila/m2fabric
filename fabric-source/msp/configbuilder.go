...
func GetLocalMspConfig(dir string, bccspConfig *factory.FactoryOpts, ID string) (*msp.MSPConfig, error) {
	...
	*/
	...
	// For PQC keys, the private key is stored as key.pem in the keystore directory
	// (SW BCCSP cannot store PQC keys). Load it directly into PrivateSigner.
	var sigid *msp.SigningIdentityInfo
	keyPemPath := filepath.Join(keystoreDir, "key.pem")
	if keyPemBytes, kerr := os.ReadFile(keyPemPath); kerr == nil {
		sigid = &msp.SigningIdentityInfo{
			PublicSigner: signcert[0],
			PrivateSigner: &msp.KeyInfo{
				KeyIdentifier: "pqc-key",
				KeyMaterial:   keyPemBytes,
			},
		}
	} else {
