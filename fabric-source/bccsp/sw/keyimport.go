...
func (ki *x509PublicKeyImportOptsKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (bccsp.Key, error) {
	...
	default:
		// PQC certificates have an unknown public key type in Go's x509 parser.
		// Return a stub key that carries the raw SPKI bytes so MSP loading succeeds.
		return &pqcPublicKey{certRaw: x509Cert.RawSubjectPublicKeyInfo}, nil
	}
}
