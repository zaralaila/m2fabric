...
// For PQC signers, the CSR is built using raw ASN.1 instead of
// x509.CreateCertificateRequest, which does not support PQC key types.
func Generate(priv crypto.Signer, req *CertificateRequest) (csr []byte, err error) {
	sigAlgo := helpers.SignerAlgo(priv)
	if sigAlgo == x509.UnknownSignatureAlgorithm {
		return nil, cferr.New(cferr.PrivateKeyError, cferr.Unavailable)
	}
	if sigAlgo == helpers.PQCSignatureAlgorithm {
		return generatePQCCSR(priv, req)
	}
...
// PQCSignerPublicKey is implemented by PQC crypto.Signer types to expose
// their raw public key bytes and OID for CSR construction.
type PQCSignerPublicKey interface {
	PQCPublicKeyBytes() []byte
	PQCOID() asn1.ObjectIdentifier
}

// generatePQCCSR builds a PKCS#10 CSR for PQC keys by constructing the
// ASN.1 structure directly, bypassing x509.CreateCertificateRequest.
// The CSR signature field is left empty (zero-length BitString) because
// Fabric CA does not verify CSR self-signatures for server-side key gen.
func generatePQCCSR(priv crypto.Signer, req *CertificateRequest) ([]byte, error) {
	pub, ok := priv.Public().(PQCSignerPublicKey)
	if !ok {
		return nil, errors.New("generatePQCCSR: signer does not implement PQCSignerPublicKey")
	}
	...
	pubBytes := pub.PQCPublicKeyBytes()
	spkiVal := spki{
		Algorithm: algorithmIdentifier{Algorithm: pub.PQCOID()},
		PublicKey: asn1.BitString{Bytes: pubBytes, BitLength: len(pubBytes) * 8},
	}
	spkiDER, err := asn1.Marshal(spkiVal)
	if err != nil {
		return nil, err
	}

	// Subject RDN
	subjectDER, err := asn1.Marshal(req.Name().ToRDNSequence())
	if err != nil {
		return nil, err
	}
	...
	// Sign the CertificationRequestInfo with the PQC key.
	// We use SHA-256 as the digest for the signature (Fabric CA verifies this).
	h := crypto.SHA256.New()
	h.Write(infoDER)
	digest := h.Sum(nil)
	sig, err := priv.Sign(rand.Reader, digest, crypto.SHA256)
	if err != nil {
		return nil, errors.New("generatePQCCSR: failed to sign CSR: " + err.Error())
	}
	...
	// Use the PQC algorithm OID as the signature algorithm
	csrVal := certificationRequest{
		CertificationRequestInfo: asn1.RawValue{FullBytes: infoDER},
		SignatureAlgorithm:       pkix.AlgorithmIdentifier{Algorithm: pub.PQCOID()},
		Signature:                asn1.BitString{Bytes: sig, BitLength: len(sig) * 8},
	}
	csrDER, err := asn1.Marshal(csrVal)
	if err != nil {
		return nil, err
	}
