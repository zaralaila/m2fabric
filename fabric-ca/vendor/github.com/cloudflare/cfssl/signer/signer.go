...
func DefaultSigAlgo(priv crypto.Signer) x509.SignatureAlgorithm {
	...
	default:
		// PQC subject keys — the CA signs with its own key (ECDSA),
		// so we return ECDSAWithSHA256 as the certificate signature algorithm.
		return x509.ECDSAWithSHA256
	}
}
...
// For PQC algorithms (ML-DSA, FN-DSA), x509.ParseCertificateRequest will fail
// because Go does not know these OIDs. In that case we parse the ASN.1 directly
// and extract the subject and public key, setting PublicKey to a *PQCPublicKeyRaw
// that carries the raw SPKI bytes for later certificate creation.
func ParseCertificateRequest(s Signer, csrBytes []byte) (template *x509.Certificate, err error) {
	csrv, err := x509.ParseCertificateRequest(csrBytes)
	if err != nil {
		// Attempt PQC CSR parsing
		var pqcErr error
		template, pqcErr = parsePQCCertificateRequest(s, csrBytes)
		if pqcErr != nil {
...
// For PQC keys (*PQCPublicKeyRaw), the SPKI bytes are used directly.
func ComputeSKI(template *x509.Certificate) ([]byte, error) {
	pub := template.PublicKey
	var encodedPub []byte
	var err error
	if pqcPub, ok := pub.(*PQCPublicKeyRaw); ok {
		encodedPub = pqcPub.RawSPKI
	} else {
...
// PQCPublicKeyRaw holds the raw SubjectPublicKeyInfo bytes and OID for a PQC
// public key extracted from a CSR. It is used as template.PublicKey so that
// the local signer's PQCCertCreator hook can embed it in the issued certificate.
type PQCPublicKeyRaw struct {
	OID    asn1.ObjectIdentifier
	RawSPKI []byte // full DER-encoded SubjectPublicKeyInfo
	RawPub  []byte // raw public key bytes (BitString content)
}

// pqcCSR is used to parse the ASN.1 structure of a PKCS#10 CSR manually.
type pqcCSR struct {
	CertificationRequestInfo pqcCSRInfo
...
// parsePQCCertificateRequest parses a CSR containing a PQC public key by
// reading the ASN.1 structure directly, bypassing x509.ParseCertificateRequest.
func parsePQCCertificateRequest(s Signer, csrBytes []byte) (*x509.Certificate, error) {
	var req pqcCSR
	rest, err := asn1.Unmarshal(csrBytes, &req)
	if err != nil || len(rest) > 0 {
		return nil, errors.New("failed to parse PQC CSR ASN.1")
	}

	// Parse the subject
	var subject pkix.RDNSequence
	if _, err := asn1.Unmarshal(req.CertificationRequestInfo.Subject.FullBytes, &subject); err != nil {
		return nil, errors.New("failed to parse PQC CSR subject")
	}
	var name pkix.Name
	name.FillFromRDNSequence(&subject)

	// Re-encode the SPKI for embedding
	spkiDER, err := asn1.Marshal(req.CertificationRequestInfo.SubjectPublicKeyInfo)
	if err != nil {
		return nil, errors.New("failed to re-encode PQC SPKI")
	}

	pub := &PQCPublicKeyRaw{
		OID:     req.CertificationRequestInfo.SubjectPublicKeyInfo.Algorithm.Algorithm,
		RawSPKI: spkiDER,
		RawPub:  req.CertificationRequestInfo.SubjectPublicKeyInfo.PublicKey.Bytes,
	}
