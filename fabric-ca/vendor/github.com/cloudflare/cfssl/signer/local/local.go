...
// PQCCertCreator is an optional hook for creating certificates with PQC public keys.
// When non-nil, sign() calls it instead of x509.CreateCertificate for unknown key types.
var PQCCertCreator func(template, parent *x509.Certificate, priv crypto.Signer) ([]byte, error)

// SetPQCCertCreator registers a function to handle PQC certificate creation.
func SetPQCCertCreator(fn func(template, parent *x509.Certificate, priv crypto.Signer) ([]byte, error)) {
	PQCCertCreator = fn
}
...
func (s *Signer) sign(template *x509.Certificate, lintErrLevel lints.LintStatus, lintIgnore map[string]bool) (cert []byte, err error) {
	...
	case *ecdsa.PublicKey, *rsa.PublicKey:
		// If the CA signing key is PQC, use PQCCertCreator even for classical subject keys.
		if PQCCertCreator != nil {
			if _, isPQC := s.priv.Public().(interface{ PQCPublicKeyBytes() []byte }); isPQC {
				certPEM, err = PQCCertCreator(template, s.ca, s.priv)
				if err != nil {
	...
	default:
		if PQCCertCreator == nil {
			return nil, cferr.Wrap(cferr.CertificateError, cferr.Unknown,
				fmt.Errorf("PQCCertCreator is nil for key type %T", template.PublicKey))
		}
		certPEM, err = PQCCertCreator(template, s.ca, s.priv)
		if err != nil {
			return nil, cferr.Wrap(cferr.CertificateError, cferr.Unknown, err)
