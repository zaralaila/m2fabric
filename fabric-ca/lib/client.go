...
func (c *Client) generateCSPSigner(cr *csr.CertificateRequest, key bccsp.Key) (crypto.Signer, bccsp.Key, error) {
	if key == nil {
		...
		// Persist PQC private key to keystore/key.pem so the client can reload it.
		if util.IsPQCAlgo(cr.KeyRequest.Algo()) && c.keyFile != "" {
			if werr := util.WritePQCPrivKeyPEM(cspSigner, c.keyFile); werr != nil {
				log.Debugf("Failed to persist PQC client key: %s", werr)
			}
		}
