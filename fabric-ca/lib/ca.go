...
func (ca *CA) getCACert() (cert []byte, err error) {
	if ca.Config.Intermediate.ParentServer.URL != "" {
		...
		// Persist PQC private key to ca-key.pem so it survives restarts.
		if util.IsPQCAlgo(csr.KeyRequest.Algo) {
			keyPath := ca.Config.CA.Keyfile
			if keyPath == "" {
				keyPath = ca.HomeDir + "/ca-key.pem"
			}
			if werr := util.WritePQCPrivKeyPEM(cspSigner, keyPath); werr != nil {
				return nil, errors.WithMessage(werr, "Failed to persist PQC CA private key")
			}
			log.Infof("PQC CA private key written to %s", keyPath)
		}
...
func (ca *CA) VerifyCertificate(cert *x509.Certificate, forceTime bool) error {
	...
	// For PQC-issued certificates, Go's x509.Verify fails because the CA cert
	// has an unknown public key algorithm. Instead, do a manual check:
	// verify the cert is in the DB (already done by caller) and check dates.
	_, err = cert.Verify(checkOpts)
	if err != nil {
		// For PQC CAs, Go x509 cannot verify the chain because the CA cert has an
		// unknown public key algorithm. Fall back to manual date + AKI check.
		if strings.Contains(err.Error(), "unknown authority") || strings.Contains(err.Error(), "unknown public key") {
			now := time.Now()
			if forceTime {
				now = cert.NotBefore.Add(time.Duration(time.Second * 30))
			}
			if now.Before(cert.NotBefore) || now.After(cert.NotAfter) {
				return errors.New("Certificate has expired or is not yet valid")
			}
			caCertPEM, cerr := ca.getCACert()
			if cerr == nil {
					...
					if caCert, perr := x509.ParseCertificate(block.Bytes); perr == nil {
						if len(cert.AuthorityKeyId) > 0 && len(caCert.SubjectKeyId) > 0 {
							if !bytes.Equal(cert.AuthorityKeyId, caCert.SubjectKeyId) {
								return errors.New("Certificate was not issued by this CA")
							}
						}
					}
				}
...
func (ca *CA) getVerifyOptions() (*x509.VerifyOptions, error) {
	...
	for len(chain) > 0 {
		...
		// PQC CA certs use unknown public key algorithms; Go's x509 parser may
		// not set IsCA even when the BasicConstraints extension is present.
		// Skip the IsCA check for certs with unknown public key types.
		if !cert.IsCA && cert.PublicKeyAlgorithm != x509.UnknownPublicKeyAlgorithm {
