...
// NewSignerFromSigner creates a Signer using a crypto.Signer (for PQC keys).
func NewSignerFromSigner(s crypto.Signer, cert []byte) (*Signer, error) {
	signer := &Signer{
		cryptoSigner: s,
		certBytes:    cert,
	}
	var err error
	signer.cert, err = util.GetX509CertificateFromPEM(signer.certBytes)
	if err != nil {
		return nil, errors.WithMessage(err, "Failed to unmarshal X509 certificate bytes")
	}
	signer.name = util.GetEnrollmentIDFromX509Certificate(signer.cert)
	return signer, nil
}
...
type Signer struct {
	// Private key
	key bccsp.Key
	// cryptoSigner is used for PQC keys that implement crypto.Signer directly
