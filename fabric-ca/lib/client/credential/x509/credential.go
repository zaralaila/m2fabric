...
func (cred *Credential) Load() error {
	...
	if err != nil {
		...
		if err != nil {
			// Second fallback: try loading as PQC key
			pqcSigner, pqcErr := util.LoadPQCSignerFromPEM(cred.keyFile)
			if pqcErr != nil {
			...
			cred.val, err = NewSignerFromSigner(pqcSigner, cert)
			if err != nil {
				return err
			}
			return nil
...
func (cred *Credential) CreateToken(req *http.Request, reqBody []byte) (string, error) {
	if cred.val.cryptoSigner != nil {
		// PQC identity — use direct crypto.Signer token generation
		return util.GenPQCToken(cred.val.certBytes, cred.val.cryptoSigner, req.Method, req.URL.RequestURI(), reqBody)
	}
