...
func processSignRequest(id string, req *signer.SignRequest, ca *CA, ctx *serverRequestContextImpl) error {
	...
	var pqcCN string
	if err != nil {
		// PQC CSR — x509 cannot parse unknown OIDs. Extract CN via raw ASN.1.
		pqcCN, err = util.ExtractCNFromCSR(block.Bytes)
		if err != nil {
			return err
		}
		log.Debugf("Processing PQC sign request: id=%s, CommonName=%s", id, pqcCN)
		if (req.Subject != nil && req.Subject.CN != id) || pqcCN != id {
			return caerrors.NewHTTPErr(403, caerrors.ErrCNInvalidEnroll, "The CSR subject common name must equal the enrollment ID")
		}
	} else if (req.Subject != nil && req.Subject.CN != id) || csrReq.Subject.CommonName != id {
		return caerrors.NewHTTPErr(403, caerrors.ErrCNInvalidEnroll, "The CSR subject common name must equal the enrollment ID")
	}
	var isForCACert bool
	if csrReq != nil {
		isForCACert, err = isRequestForCASigningCert(csrReq, ca, req.Profile)
		if err != nil {
			return err
		}
		// Check the CSR input length (skip for PQC CSRs parsed via raw ASN.1)
