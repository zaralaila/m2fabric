...
func CreateToken(csp bccsp.BCCSP, cert []byte, key bccsp.Key, method, uri string, body []byte) (string, error) {
	...
	*/
	...
	default:
		// PQC public key — use PQC token generation if a PQC signer is available.
		if pqcKey, ok := key.(PQCSigningKey); ok {
			token, err = GenPQCToken(cert, pqcKey.CryptoSigner(), method, uri, body)
			if err != nil {
				return "", err
			}
		}
...
func VerifyToken(csp bccsp.BCCSP, token string, method, uri string, body []byte, compMode1_3 bool) (*x509.Certificate, error) {
	...
	// For PQC certificates, verify the token using the raw public key directly.
	if x509Cert.PublicKeyAlgorithm == x509.UnknownPublicKeyAlgorithm {
		if verifyErr := VerifyPQCToken(x509Cert, sig, sigString); verifyErr != nil {
