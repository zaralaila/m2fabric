...
func BccspBackedSigner(caFile, keyFile string, policy *config.Signing, csp bccsp.BCCSP) (signer.Signer, error) {
	...
	if err != nil {
		...
		if err != nil {
			// Fallback: try loading as a PQC private key
			pqcSigner, pqcErr := LoadPQCSignerFromPEM(keyFile)
			if pqcErr != nil {
...
// For PQC algorithms (mldsa44, mldsa65, fndsa512) it delegates to
// BCCSPPQCKeyRequestGenerate which uses the pqc-bccsp plugin directly.
func BCCSPKeyRequestGenerate(req *csr.CertificateRequest, myCSP bccsp.BCCSP) (bccsp.Key, crypto.Signer, error) {
	...
	if req.KeyRequest != nil && IsPQCAlgo(req.KeyRequest.Algo()) {
		return BCCSPPQCKeyRequestGenerate(req.KeyRequest.Algo())
	}
